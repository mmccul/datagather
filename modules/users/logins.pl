...
# Manually, clumsily, create XML output.  Why?  Because I cannot count on
# a useful XML writer, and XML is much easier to create than JSON.
if ( $opts{x} ) {
    my $userent;
    printf ("<userlist>\n");
    for $userent (@userlist) {
        ...
        for my $key (keys (%{$userent})) {
            if ( ref ${$userent}{$key} eq "ARRAY") {
                # array here
                printf("        <%s>\n",$key);
                foreach my $element (@{${$userent}{$key}}) {
                    printf("            <item>%s</item>\n",$element);
                }
                printf("        </%s>\n",$key);
            } else {
